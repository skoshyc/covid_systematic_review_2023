...
specificity_logit_test_subgp <- metaprop(subset_group$TN, subset_group$TN+subset_group$FP, 
                              comb.fixed = FALSE, comb.random = TRUE,
                              sm = "PLOGIT", method.ci = "CP", 
                              studlab = subset_group$Identifier,
                              byvar = subset_group$test_name)
print(specificity_logit_test_subgp, digits= 3)
png(file = "forestplot_overall_specificities_test_subgp_freq5.png", width = 3000, height = 6000, res = 300)
forest(specificity_logit_test_subgp, digits= 3,
       xlab= "Specificity",leftlabs = c("Source","Specificity (95% CI)"),layout = "JAMA",
...
library(dmetar)
library(dplyr)
outlier=find.outliers(specificity_logit_test_subgp)
print(outlier)

new_subset=subset_group[!(subset_group$Identifier %in%
                            c("Alghounaim, et. al. Dec 2021", "Amer, et. al. Oct 2021", 
                              "Boum, et. al. May 2021", "Korenkov, et. al. Aug 2021", 
                              "Turcato, et. al. Dec 2020", "Allan-Blitz, et. al. Sep 2021",
                               "Amer, et. al.Mar 2022", "Cottone, et. al. May 2022", 
                              "Freire, et. al. Jun 2022", "González-Fiallo, et. al. Apr 2022", 
                              "Jakobsen, et. al. Oct 2021", "Jeewandara, et. al. Mar 2022", 
                              "Paap, et. al. Nov 2021", "Pandey, et. al. Aug 2021", 
                              "Sitoe, et. al. Feb 2022", "Thirion-Romero, et. al. Oct 2021")),]
specificity_logit_test_subgp_new <- metaprop(new_subset$TN, new_subset$TN+new_subset$FP, 
                                         comb.fixed = FALSE, comb.random = TRUE,
                                         sm = "PLOGIT", method.ci = "CP", 
                                         studlab = new_subset$Identifier,
                                         byvar = new_subset$test_name)
print(specificity_logit_test_subgp_new, digits= 3)
png(file = "forestplot_overall_specificities_test_subgp_woOutliers.png", width = 3000, height=6000,
    res = 300)
forest.meta(specificity_logit_test_subgp, digits= 3,
       xlab= "Specificity",leftlabs = c("Source","Specificity (95% CI)"),layout = "JAMA",
       col.by="black", addrow.overall = TRUE, addrow.subgroups = TRUE) 
dev.off()
...
#Specificity analysis for symptomatic 
...
specificity_logit_symp_subgp <- metaprop(subset_symp$TN, subset_symp$TN+subset_symp$FP, 
                                         comb.fixed = FALSE, comb.random = TRUE,
                                         sm = "PLOGIT", method.ci = "CP", 
                                         studlab = subset_symp$Identifier,
                                         byvar = subset_symp$test_name)
print(specificity_logit_symp_subgp)
png(file = "forestplot_specificity_symp.png", width = 3000, height = 3000, res = 300)
forest.meta(specificity_logit_symp_subgp, digits= 3,
            xlab= "Specificity",leftlabs = c("Source","Specificity (95% CI)"),layout = "JAMA",
            col.by="black", addrow.overall = TRUE, addrow.subgroups = TRUE)
dev.off()
...
asymp1=asymp[which(is.na(asymp$`True Negative (TN)`)!=TRUE),]
...
specificity_logit_asymp_subgp <- metaprop(subset_asymp$TN, subset_asymp$TN+subset_asymp$FP, 
                                         comb.fixed = FALSE, comb.random = TRUE,
                                         sm = "PLOGIT", method.ci = "CP", 
                                         studlab = subset_asymp$Identifier,
                                         byvar = subset_asymp$test_name)
print(specificity_logit_asymp_subgp)
png(file = "forestplot_specificity_asymp.png", width = 3000, height = 3000, res = 300)
forest.meta(specificity_logit_asymp_subgp, digits= 3,
            xlab= "Specificity",leftlabs = c("Source","Specificity (95% CI)"),layout = "JAMA",
            col.by="black", addrow.overall = TRUE, addrow.subgroups = TRUE)
dev.off()
