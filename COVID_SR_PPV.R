...
PPV_logit_test_subgp <- metaprop(subset_group$TP, subset_group$TP+subset_group$FP, 
                                         comb.fixed = FALSE, comb.random = TRUE,
                                         sm = "PLOGIT", method.ci = "CP", 
                                         studlab = subset_group$Identifier,
                                         byvar = subset_group$test_name)
print(PPV_logit_test_subgp, digits= 3)
png(file = "forestplot_overall_PPV_test_subgp_freq5.png", width = 3000, height = 6000, res = 300)
forest(PPV_logit_test_subgp, digits= 3,
       xlab= "PPV",leftlabs = c("Source","PPV (95% CI)"),layout = "JAMA",
...
library(dmetar)
library(dplyr)
outlier=find.outliers(PPV_logit_test_subgp)
print(outlier)

new_subset=subset_group[!(subset_group$Identifier %in%
                            c("Alghounaim, et. al. Dec 2021", "Boum, et. al. May 2021", 
                              "Fernandez-Montero, et. al. July 2021", "Jakobsen, et. al. June 2021", 
                              "Korenkov, et. al. Aug 2021", "Turcato, et. al. Dec 2020",
                              "Allan-Blitz, et. al. Sep 2021", "Cottone, et. al. May 2022", 
                              "González-Fiallo, et. al. Apr 2022", "Jeewandara, et. al. Mar 2022",
                              "Paap, et. al. Nov 2021", 
                              "Pandey, et. al. Aug 2021", "Turcato, et. al. Jan 2022")),]
PPV_logit_test_subgp_new <- metaprop(new_subset$TP, new_subset$TP+new_subset$FP, 
                                             comb.fixed = FALSE, comb.random = TRUE,
                                             sm = "PLOGIT", method.ci = "CP", 
                                             studlab = new_subset$Identifier,
                                             byvar = new_subset$test_name)
print(PPV_logit_test_subgp_new, digits= 3)
png(file = "forestplot_overall_PPV_test_subgp_woOutliers.png", width = 3000, height=6000,
    res = 300)
forest.meta(PPV_logit_test_subgp, digits= 3,
            xlab= "PPV",leftlabs = c("Source","PPV (95% CI)"),layout = "JAMA",
            col.by="black", addrow.overall = TRUE, addrow.subgroups = TRUE) 
dev.off()
...
PPV_logit_symp_subgp <- metaprop(subset_symp$TP, subset_symp$TP+subset_symp$FP, 
                                         comb.fixed = FALSE, comb.random = TRUE,
                                         sm = "PLOGIT", method.ci = "CP", 
                                         studlab = subset_symp$Identifier,
                                         byvar = subset_symp$test_name)
print(PPV_logit_symp_subgp)
png(file = "forestplot_PPV_symp.png", width = 3000, height = 3000, res = 300)
forest.meta(PPV_logit_symp_subgp, digits= 3,
            xlab= "PPV",leftlabs = c("Source","PPV (95% CI)"),layout = "JAMA",
            col.by="black", addrow.overall = TRUE, addrow.subgroups = TRUE)
dev.off()
...
PPV_logit_asymp_subgp <- metaprop(subset_asymp$TP, subset_asymp$TP+subset_asymp$FP, 
                                          comb.fixed = FALSE, comb.random = TRUE,
                                          sm = "PLOGIT", method.ci = "CP", 
                                          studlab = subset_asymp$Identifier,
                                          byvar = subset_asymp$test_name)
print(PPV_logit_asymp_subgp)
png(file = "forestplot_PPV_asymp.png", width = 3000, height = 3000, res = 300)
forest.meta(PPV_logit_asymp_subgp, digits= 3,
            xlab= "PPV",leftlabs = c("Source","PPV (95% CI)"),layout = "JAMA",
            col.by="black", addrow.overall = TRUE, addrow.subgroups = TRUE)
dev.off()
